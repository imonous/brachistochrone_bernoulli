...
def rand_x():
    ...
    xs = lambda: random.uniform(1e-10, 1)  # small
    xl = lambda: random.uniform(1e3, 1e9)  # large
    return xs() if random.randint(0, 1) else xl()
...
def test_angle_setter_result(rand_x, angle):
    r = BernoulliRay(rand_x, angle)
...
def test_propagate_refract_results(rand_x, n1, n2, incidence, expected):
    v1, v2 = LightMedium.n_to_v(n1), LightMedium.n_to_v(n2)
    m1, m2 = LightMedium(v1), LightMedium(v2)
    r = BernoulliRay(rand_x, math.pi / 2 - incidence)
    r.propagate(m1, m2)
    assert pytest.approx(r.get_incidence()) == expected
